...
if __name__ == '__main__':  # Check to disallow importing
    ...
    output = key_randomizer.password_generator()  # saving the output to output variable 
    print(output+'\nThis is your key,this key will also be saved in a text file named as password.txt\nThanks for '
                 'using our tool !\n(Ignore the text shown below)')
    key_randomizer.process_completed()  # To play the audio file for process completion
    key_randomizer.password_saver()
    key_randomizer.password_saved()  # To play the audio file for process completion
